...
router.post('/register', async ({ body, session }, res, next) => {
  try {
    const account = await create(body)
    session.account = account
    res.redirect('/settings')
  } catch (err) {
    next(err)
...
router.get('/authorize', requireSession, (req, res, next) => {
  try {
    ...
    res.redirect(`${req.session.requestingService.redirectUri}?access_token=${token}&account_id=${accountId}`)
