...
async function create (account) {
  ...
  try {
    ...
    await conn.query('INSERT INTO accounts(id, account_key, pds_provider, pds_credentials) VALUES($1, $2, $3, $4)', [
      id,
...
async function get (accountId) {
  ...
  const conn = await connect()
  try {
    const result = await conn.query('SELECT id, account_key, pds_provider, pds_credentials FROM accounts WHERE id=$1', [accountId])
